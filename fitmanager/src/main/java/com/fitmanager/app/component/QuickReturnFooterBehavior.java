...
public class QuickReturnFooterBehavior extends CoordinatorLayout.Behavior<View> {
    ...
    private final int HIDING_THRESHOLD = 5;
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, View child, View target, int dx, int dy, int[] consumed) {
        ...
        if (mDySinceDirectionChange > SCROLL_THRESHOLD
                && child.getVisibility() == View.VISIBLE
                && !mIsHiding) {
//            int itemCount = NumberUtils.toInt(target.getTag().toString(), 0);
            int itemCount = ((RecyclerView) target).getAdapter().getItemCount();
            if (itemCount > HIDING_THRESHOLD) {
                hide(child);
            }
